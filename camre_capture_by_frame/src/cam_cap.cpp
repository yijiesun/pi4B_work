		...
		if(!frame_cur[newFramePos].empty())
		{
			...
			//if (tt - lastTime >= CAPFPS)
			{
				...
				{//按天创建主文件夹
					...
					if (ret < 0)
					{
						cout << "\031[41m" << log_Time() << "Could not create directory " << dirsDay.str() << "ret " << ret << "\033[0m" << endl;
						//return EXIT_FAILURE;
					}
				...
				if (fileDay != t->tm_mday)
				{
					...
					if (ret < 0)
					{
						cout << "\031[41m" << log_Time() << "Could not create directory " << dirsDay.str() << "\033[0m" << endl;
						//return EXIT_FAILURE;
					}
					dirVectorSaved.push_back(dirsDay.str());
					ioFileAll<< dirsDay.str()<<endl;
			...
			if (pushToAVICntRear == 0 && isCatchMove)
			{
				...
				{ //创建天文件夹里面的子文件夹
					...
					if (ret < 0)
					{
						cout << "\031[41m" << log_Time() << "Could not create directory " << dirsSon.str() << "\033[0m" << endl;
